...
<a href="<?php echo url('Sellernavigation/navigation_add'); ?>" class="dssc-btn dssc-btn-green" title="<?php echo \think\Lang::get('store_navigation_new'); ?>"><?php echo \think\Lang::get('store_navigation_new'); ?></a>
...
<table class="dssc-default-table">
    <thead>
        <tr>
            <th class="w60"><?php echo \think\Lang::get('store_goods_class_sort'); ?></th>
            <th class="tl"><?php echo \think\Lang::get('store_navigation_name'); ?></th>
            <th class="w110"><?php echo \think\Lang::get('ds_handle'); ?></th>
        </tr>
    </thead>
    <tbody>
        <?php if($navigation_list): if(is_array($navigation_list) || $navigation_list instanceof \think\Collection || $navigation_list instanceof \think\Paginator): if( count($navigation_list)==0 ) : echo "" ;else: foreach($navigation_list as $key=>$nav): ?>
        <tr class="bd-line">
            <td><?php echo $nav['storenav_sort']; ?></td>
            <td class="tl"><dl class="goods-name"><dt><a href="<?php if($nav['storenav_url'] == ''): ?><?php echo url('Store/article',array('store_id' =>session('store_id'),'storenav_id'=>$nav['storenav_id'])); else: ?><?php echo $nav['storenav_url']; endif; ?>"><?php echo $nav['storenav_title']; ?></a></dt></dl></td>
            <td class="dssc-table-handle">
                <span><a href="<?php echo url('Sellernavigation/navigation_edit',array('storenav_id' => $nav['storenav_id'])); ?>" class="btn-blue"><i class="fa fa-pencil-square"></i><p> <?php echo \think\Lang::get('ds_edit'); ?></p></a></span>
                <span> <a href="javascript:ds_ajaxget_confirm('<?php echo url('Sellernavigation/navigation_del',['storenav_id'=>$nav['storenav_id']]); ?>','<?php echo \think\Lang::get('ds_ensure_del'); ?>');" class="btn-red" ><i class="fa fa-trash-o"></i><p><?php echo \think\Lang::get('ds_del'); ?></p></a></span>
            </td>
        </tr>
